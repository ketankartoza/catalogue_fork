...
def ingest(
    ...
    log_message('Scanning folders in %s' % source_path)
    ...
    for myFolder in glob.glob(os.path.join(source_path, '*')):
        ...
        try:
            ...
            log_message('Radiometric resolution: %s' % radiometric_resolution)
            quality = get_quality()
