...
def parseDateTime(theDate):
    ...
    Args:
	    ...
	    :IIF samp:`1989-05-03T07:30:05.000`
    Returns:
...
def ingest(
        ...
        theSourceDir= '/home/web/catalogue/django_project/catalogue/tests/sample_files/landsat/',
        theVerbosityLevel=2,
    ...
    Args:
        ...
        * theLicense - (Optional) Defaults to 'SANSA Free License',
	...
	myElement = myDom.getElementsByTagName('license')[0]
        theLicense = myElement.firstChild.nodeValue
        logMessage('License: %s' % theLicense, 2) 
	myLicenseMapping = {
            'USGS free distribution license': 'SANSA Free License'
   	         	    }
        theSANSALicense = License.objects.get(
              name=myLicenseMapping[myLicenseName])
        logMessage('License: %s' % theLicense, 2)
	...
	logMessage('License: %s' % myLicense)
        ...
        theSoftware = myElement.firstChild.nodeValue
        logMessage('Software: %s' % theSoftware, 2) 
	mySoftwareMapping = {
            'Pinkmatter Landsat Processor': 'LPGS'
   	         	    }
        theSoftware = CreatingSoftware.objects.get(
              name=mySoftwareMapping[mySoftwareName])
	...
	# Get the spectral mode 
        ...
        mySpectralMode = myElement.firstChild.nodeValue
...
# *** Continue from here
	...
	# Get the spatial coverage
        myElement = myDom.getElementsByTagName('spatialCoverage.boundingPolygon.latitiude')[0]
        myUpperLeftLat = myElement.firstChild.nodeValue
        logMessage('Upper Left Lat: %s' % myUpperLeftLat, 2)

        myElement = myDom.getElementsByTagName('spatialCoverage.boundingPolygon.longitude')[0]
