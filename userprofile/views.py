...
def register(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if form.cleaned_data.get('email', u''):
                newuser.email = form.cleaned_data.get('email')
                EmailValidation.objects.add(user=newuser, email=newuser.email)

            newuser.firstname = form.cleaned_data.get('firstname', u'')
            newuser.last_name = form.cleaned_data.get('surname', u'')

            newuser.save()

            # create the extended profile object
            profile = Profile.objects.create(user=newuser,
                      firstname = form.cleaned_data.get('firstname'),
                      surname = form.cleaned_data.get('surname'),
                      url = form.cleaned_data.get('url', u''),
                      about = form.cleaned_data.get('about', u''),
                      address1 = form.cleaned_data.get('address1'),
                      address2 = form.cleaned_data.get('address2'),
                      address3 = form.cleaned_data.get('address3', u''),
                      address4 = form.cleaned_data.get('address4', u''),
